...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname 32.4v2) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
  ...
  (local (; adds spaces between each word
          ...
          (define (fmt/a cntnts accum-lines accum-words)
            (local ((define cur-line-lgth (image-width (editor-text (strings->1strings accum-words))))
                    (define rev-line (reverse accum-words)))
              (cond
                [(and (empty? cntnts) (empty? accum-lines)) accum-words]
                [(empty? cntnts) (add-to-end accum-words accum-lines)]
                [(> w cur-line-lgth) (fmt/a (rest cntnts) accum-lines (add-to-end (first cntnts) accum-words))]
                [(equal? w cur-line-lgth) (fmt/a (rest cntnts) (add-to-end accum-words accum-lines) '())]
                [else (fmt/a (cons (first rev-line) cntnts) (add-to-end (reverse (rest rev-line)) accum-lines) '())])))
          (define words/line (fmt/a words-w-spaces '() '()))
...
(fmt 120 "ttt.txt" "tttnew.v2.txt")
;TODO - fix v2 of fmt. v2 of fmt does the same as the original fmt except it accumulates the file from beginning to end as opposed to reverse order like the original fmt does.
; this will make it so the last line of the out-file will be cut short as opposed to the first line.