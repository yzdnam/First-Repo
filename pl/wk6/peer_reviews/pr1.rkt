  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([cls (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? cls)
               (let* ([f (closure-fun cls)]
                      [env2 (cons (cons (fun-formal f) arg)
                                  (closure-env cls))])
                 (eval-under-env (fun-body f)
                                 (if (fun-nameopt f)
                                     (cons (cons (fun-nameopt f) cls)
                                           env2)
                                     env2)))
  ...
  (fun #f "fx"
       (fun "muplmap" "xs"
            (ifaunit (var "xs") (aunit)
                     (apair (call (var "fx") (fst (var "xs")))
                            (call (var "muplmap") (snd (var "xs"))))))))

(define mupl-mapAddN 
  (fun #f "i"
       (fun "mapAddN" "loi"
            (call (call mupl-map (fun #f "x" (add (var "x") (var "i")))) (var "loi")))))
