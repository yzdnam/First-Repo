...
def damage_knight(dam)
  ...
  then
    ...
    then
      dam_to_ap = @ap
      @ap = 0
      self.damage_knight(dam - dam_to_ap)
    else @ap = (@ap - dam)
    end
  elsif @ap == 0
    @hp = (@hp - dam) 
...
def encounter_floortrap floortrap
  damage_knight(floortrap.dam)
end

def encounter_monster monster
  damage_knight(monster.dam)
end
