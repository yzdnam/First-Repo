...
//    t.checkExpect(l, al);
  ...
  void testHeapClass(Tester t) {
    initList();
    h = new Heap<Integer>(l, new LessThan());
    h.add(10);
    t.checkExpect(h.heap, al);
    h.remove();
    t.checkExpect(h.heap, al);
  }
...
class Heap<T> {
  ...
  IComparator<T> comp;
  ArrayUtils au;
  Heap(ArrayList<T> heap, IComparator<T> comp) {
    new ArrayUtils().buildHeap(heap, comp);
    this.heap = heap;
    this.comp = comp;
  ...
  void add(T item) {
    this.heap.add(item);
    au.buildHeap(heap, comp);
  }
  
  void add(ArrayList<T> itemList) {
    for (T t : itemList) {
      this.add(t);
    }
  }
...
class ArrayUtils {
  ...
  public int weight(ArrayList<Edge> arr) {
    int result = 0;
    for (Edge e : arr) {
  ...
  public boolean allVertices(ArrayList<Edge> edges, ArrayList<Vertex> vertices) {
    ArrayList<Vertex> verticesLeft = new ArrayList<Vertex>(vertices);
    for (Edge e : edges) {
      if (verticesLeft.contains(e.from)) {
        verticesLeft.remove(e.from);
      }
      if (verticesLeft.contains(e.to)) {
        verticesLeft.remove(e.to);
      }
    }
...
class LessThan implements IComparator<Integer> {
  public int compare(Integer i, Integer j) {
    return j - i;
  }
}