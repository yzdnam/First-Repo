...
//    t.checkExpect(uhl, new ArrayList<Integer>());
  ...
  void testReverseQueue(Tester t) {
    initList();
    au = new ArrayUtils();
    au.buildHeap(l, new LessThan());
    t.checkExpect(l, new ArrayList<Integer>());
    au.buildHeap(al, new GreaterThan());
    t.checkExpect(al, new ArrayList<Integer>());
  }
}
...
class ArrayUtils {
  ...
  public <T> int weight(ArrayList<WeightedData<T>> arr) {
    int ans = 0;
    for (WeightedData<T> w : arr) {
      ans = w.weight + ans;
    }
    return ans;
  }
